...
BOT_TOKEN = "5463230148:AAGMZvj1MlgLmxyaGPC7kFZNy8YkYDD0PyU"
...
@bot.on_message(filters.command(["throw"])&  (filters.chat(sudo_groups)))
async def account_login(bot: Client, m: Message):
 editable = await m.reply_text("**Hello Sir I'm Ready to Use \nSend /txt download and for classplus send /clps  for /dhurina for /vision**")

@bot.on_message(filters.command(["txt"])&  (filters.chat(sudo_groups)))
async def account_login(bot: Client, m: Message):
	global cancel
	cancel = False
	editable = await m.reply_text("**Send Text file having Urls**")
	...
	try:    
		...
		os.remove(x)
		return
	editable = await m.reply_text(f"Total links in this txt **{len(links)}**\n\nSend From where you want to download initial is **0**")
	...
	try:
		...
		arg = 0
	editable = await editable.edit("**Enter Video Name**")
	...
	editable = await editable.edit("Now send me Mr. the **Thumb url**\nEg : ```https://telegra.ph/file/cef3ef6ee69126c23bfe3.jpg```\n\nor Send **no**")
...
async def account_login(bot: Client, m: Message):
	...
	editable = await m.reply_text("Send txt file having Urls")
	input: Message = await bot.listen(editable.chat.id)
	x = await input.download()
	await input.delete(True)
	...
	editable = await m.reply_text(
		f"Total links found in this txt file **{len(links)}**\n\nSend From where you want to download initial is **0**"
	)
	input1: Message = await bot.listen(editable.chat.id)
	raw_text = input1.text
